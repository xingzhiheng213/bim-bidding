...
def build_analyze_messages(
    ...
    When ``semantic_overrides`` is set, ``analyze_user`` is the full user template
    (including ``{{#context#}}``); otherwise built-in constants are used.
    """
    from app.semantic_slots import pick_semantic_slot_text

    system_text = pick_semantic_slot_text("analyze_system", semantic_overrides)
    user_tmpl = pick_semantic_slot_text("analyze_user", semantic_overrides)
    user_content = user_tmpl.replace(ANALYZE_CONTEXT_PLACEHOLDER, extracted_text.strip())
    return [
...
2. key_requirements：字符串数组，须响应的要点清单（响应要点），每项一条。
...
若某类信息在分析结果中未提及，则 project_info 可为空对象 {}，key_requirements 或 risk_points 可为空数组 []，可选键可省略或为空数组。不要省略前三键。"""
...
{"project_info":{"name":"...","scale":"...","location":"..."},"key_requirements":["要点1","要点2"],"risk_points":["风险1","风险2"],"scoring_items":["评分项1（X分）"],"construction_goals":["总体目标...","创优目标..."],"standards_refs":["GB/T 51212-2016"],"deliverables":["设计阶段：...","施工阶段：..."]}
"""


def build_params_messages(
    analyze_text: str,
    semantic_overrides: dict[str, str] | None = None,
) -> list[dict]:
    ...
    When ``semantic_overrides`` is set, params_system / params_user come from profile.
    """
    from app.semantic_slots import pick_semantic_slot_text

    system_text = pick_semantic_slot_text("params_system", semantic_overrides)
    user_tmpl = pick_semantic_slot_text("params_user", semantic_overrides)
    user_content = user_tmpl.replace(PARAMS_CONTEXT_PLACEHOLDER, analyze_text.strip())
    return [
        {"role": "system", "content": system_text},
        {"role": "user", "content": user_content},
    ]
...
FRAMEWORK_REQUIREMENTS_PLACEHOLDER = "{{#key_requirements#}}"
...
FRAMEWORK_USER_TEMPLATE = """请根据以下信息生成**标书的完整标题框架**（一级 第X章 + 二级 X.1、X.2 + 三级 X.1.1、X.1.2），确保覆盖评分项与招标要点。
...
# 有用户要点时：仅「当前框架 + 要点」；{current_framework_str}、{points_str} 为运行时注入
FRAMEWORK_USER_EXTRA_POINTS_TEMPLATE = """当前框架：
...
def build_framework_messages(
    ...
    - If extra_points 存在：只给「当前框架 + 用户要点」，不掺入分析/响应要点/知识库，减少干扰。
    ...
    if extra_points:
        ...
        user_content = FRAMEWORK_USER_EXTRA_POINTS_TEMPLATE.format(
            current_framework_str=current_framework_str,
            points_str=points_str,
        )
        sys_pts = pick_semantic_slot_text("framework_system_points", semantic_overrides)
        return [
            {"role": "system", "content": sys_pts},
            {"role": "user", "content": user_content},
        ]
    # 无用户要点：照旧完整生成（招标分析 + 响应要点 + 技术评分项 + 知识库）
    ...
    user_content = (
        ...
        .replace(FRAMEWORK_REQUIREMENTS_PLACEHOLDER, requirements_str)
    ...
    sys_full = pick_semantic_slot_text("framework_system", semantic_overrides)
    return [
...
# 运行时注入：chapter_full_name、analyze_excerpt（已截断）、requirements_str、risk_str、scoring_str
CHAPTER_OUTLINE_USER_TEMPLATE = """请为以下章节生成小节大纲：
...
def build_chapter_outline_messages(
    ...
    scoring_str = "\n".join(f"- {s}" for s in scoring_items) if scoring_items else "（无）"
    user_content = CHAPTER_OUTLINE_USER_TEMPLATE.format(
        chapter_full_name=chapter_full_name,
        analyze_excerpt=analyze_text.strip()[: config.CHAPTER_OUTLINE_ANALYZE_MAX_LEN],
        requirements_str=requirements_str,
        risk_str=risk_str,
        scoring_str=scoring_str,
    )
    sys_text = pick_semantic_slot_text("chapter_outline_system", semantic_overrides)
    return [
...
- 核心技术章节（BIM、实施、技术方案、应用、设计）：每小节约2000-3000字，含技术路线、关键措施、质量控制。
- 管理保障章节（组织、管理、保障、计划、团队）：每小节约1200-1800字，含组织结构、职责分工、保障措施。
...
- **自称与对方**：投标人自称以「我方」为主，仅在必要时使用，避免高频出现「我方」导致模板化；招标方称「建设单位」或「业主」，与招标文件用语保持一致。
...
- **禁止使用**：绝对保证、100%保证、完全确保、零风险、绝不、承诺…一定、无条件保证、必须…一定、必须…确保、郑重承诺、我方承诺…一定 等过度承诺表述，以免履约时被认定为未兑现承诺。
...
[段落叙述为主，辅以（1）（2）（3）…列举（列举仅在场景确实符合3-5个平行的技术要点时使用）和表格。]
...
# 运行时注入：chapter_full_name、outline_text、requirements_str、project_str、risk_str、scoring_str、context_text
CHAPTER_CONTENT_USER_TEMPLATE = """请根据以下信息撰写完整的章节内容：
...
3. 紧扣招标文件中的评分细则和技术要求
4. 优先采用知识库中的成熟表述与技术路线作为主要参考，但须做项目化改写，凡涉及项目名称、规模、地点、指标处必须与本项目一致，不得出现他项目信息
5. 段落叙述是主流（80-90%），列举和表格是辅助（10-20%）
6. 表格书写：表头行、分隔行、数据行之间**不得空行**（必须连续书写），不要用代码块（\`\`\`）包裹表格，以便正确导出为 Word
7. **禁止在正文与小标题中加粗**：段落、## X.1 / ### X.1.1 标题、（1）（2）分点等均不要使用 **文字** 或 __文字__；仅表格单元格内（如合计行）可保留加粗
8. 不要使用机械的1、2、3、4编号，不要使用"首先、其次、然后"等口语化表述
9. 避免空洞表述和过度格式化，内容要有技术深度"""
...
def build_chapter_content_messages(
    ...
    risk_str = "\n".join(f"- {r}" for r in risk_points) if risk_points else "（无）"
    scoring_str = "\n".join(f"- {s}" for s in scoring_items) if scoring_items else "（无）"
    user_content = CHAPTER_CONTENT_USER_TEMPLATE.format(
        chapter_full_name=chapter_full_name,
        ...
        risk_str=risk_str,
        scoring_str=scoring_str,
        context_text=context_text.strip() if context_text else "（无）",
    )
    sys_text = pick_semantic_slot_text("chapter_content_system", semantic_overrides)
    return [
        {"role": "system", "content": sys_text},
        {"role": "user", "content": user_content},
    ]
...
# 运行时注入：chapter_full_name、current_content、points_str
CHAPTER_REGENERATE_USER_TEMPLATE = """当前章节：{chapter_full_name}
...
def build_chapter_regenerate_messages(
    ...
    user_content = CHAPTER_REGENERATE_USER_TEMPLATE.format(
        chapter_full_name=chapter_full_name,
        current_content=current_content.strip(),
        points_str=points_str,
    )
    sys_text = pick_semantic_slot_text("chapter_regenerate_system", semantic_overrides)
    return [
        {"role": "system", "content": sys_text},
        {"role": "user", "content": user_content},
    ]
...
REVIEW_PARAMS_CONTEXT_PLACEHOLDER = "{{#params_review_context#}}"
REVIEW_KB_CONTEXT = "{{#kb_context#}}"

REVIEW_SYSTEM_SEMANTIC = """你是BIM技术标校审专家，对单章正文进行质量与合规校审。

## 不审查项（请勿作为问题提出）

- **评分响应标注**：正文中以括号形式出现的「（评分响应：…）」或「（（评分响应：…））」为本项目规定的格式要求，用于标注该段所响应的评分项，导出 Word 时会转为批注。此类文字**不是**模板化、引导性废话，**不要**将其列为套路或建议（如「章节开头使用大量括号标注评分响应…建议删除或整合」等）。仅当括号内的评分响应内容本身存在事实错误、与招标不符或过于空泛时，才可针对**内容**提出建议，且不要建议删除该格式本身。
...
3. **套路**：空话、模板化表述、与招标无关的泛化内容，缺乏针对本项目的具体表述。（不含上述评分响应标注格式。）
4. **建议**：可优化表述、建议补充依据、增强针对性等改进建议（可选）。（不含对「评分响应」括号格式本身的删除/整合建议。）"""

REVIEW_OUTPUT_FORMAT_SPEC = """## 输出格式
...
REVIEW_SYSTEM = REVIEW_SYSTEM_SEMANTIC + "\n\n" + REVIEW_OUTPUT_FORMAT_SPEC

REVIEW_USER_TEMPLATE = """请对以下章节正文进行校审，对照招标分析与参数要求，按系统提示与输出格式要求仅输出 JSON 数组。注意：正文中的「（评分响应：…）」为规定格式，勿将其列为套路或建议删除。
...
def build_review_messages(
    ...
    chapter_content: str,
    analyze_text: str,
    params_review_context: str,
    kb_context: str,
    semantic_overrides: dict[str, str] | None = None,
) -> list[dict]:
    """Build [system, user] messages for single-chapter review step.

    System content is custom ``review_system`` semantic (persona + dimensions) plus
    ``REVIEW_OUTPUT_FORMAT_SPEC``, matching built-in ``REVIEW_SYSTEM`` composition.
    """
    from app.semantic_slots import pick_semantic_slot_text
    ...
    params_str = (params_review_context or "").strip() or "（无）"
    ...
    user_content = (
        ...
        .replace(REVIEW_PARAMS_CONTEXT_PLACEHOLDER, params_str)
        .replace(REVIEW_KB_CONTEXT, kb_str)
    )
    semantic = pick_semantic_slot_text("review_system", semantic_overrides)
    system_content = semantic + "\n\n" + REVIEW_OUTPUT_FORMAT_SPEC
    return [
