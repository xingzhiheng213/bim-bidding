...
JSON 必须包含以下三个键（不可省略、不可改名）；可选第四键：
...
若某类信息在分析结果中未提及，则 project_info 可为空对象 {}，bim_requirements 或 risk_points 可为空数组 []。不要省略前三键。"""

PARAMS_USER_TEMPLATE = """从以下招标文件分析结果中提取结构化信息，形成 JSON（仅输出 JSON，不要代码块包裹或前后文字）：

""" + PARAMS_CONTEXT_PLACEHOLDER + """

输出格式示例（仅作结构参考，请按实际内容填写；scoring_items 为可选键）：
{"project_info":{"name":"...","scale":"...","location":"..."},"bim_requirements":["要求1","要求2"],"risk_points":["风险1","风险2"],"scoring_items":["评分项1（X分）","评分项2（X分）"]}
"""
...
- **层级约定**：最多三级。一级小节用 X.1、X.2；若某小节下需再分，用 X.1.1、X.1.2（X 为章节号）。
- 每行一个小节/子节，格式示例：
...
**•列举是辅助（5-10%）**：仅在场景符合3-5个平行的技术要点时使用。
**表格是辅助（5-10%）**：软硬件配置、人员配置、标准对照等多维度信息；每2000字中1-2个表格为宜。表格**必须使用标准 Markdown 语法**：表头行 `| 列1 | 列2 |`，分隔行 `|---|---|`，数据行同表头格式，以便导出为 Word 工整表格。

## 标题与段落格式（必须遵守）

- **标题层级**：最多三级。二级用 `## X.1 小节标题`，三级用 `### X.1.1 子节标题`（如 2.2.1 策划与标准制定阶段、2.2.2 方案设计阶段）。三级之下若再分点，用 **（1）（2）（3）…** 单独成行，空一行后再写正文。
...
7. 不要使用机械的1、2、3、4编号，不要使用"首先、其次、然后"等口语化表述
8. 避免空洞表述和过度格式化，内容要有技术深度"""
    return [
